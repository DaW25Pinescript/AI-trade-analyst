...
Freeze is a **formally gated commitment action**, not a lightly gated UI convenience.
Both frontend and backend must enforce the same gate independently.
...
Backend MUST independently validate the same conditions and reject freeze attempts
that bypass frontend gating. The 409 conflict path remains live regardless of
frontend state.
...
`ui/src/workspaces/journey/adapters/journeyViewModel.ts`
— `buildJourneyWorkspaceViewModel()`, `canFreeze` field.
